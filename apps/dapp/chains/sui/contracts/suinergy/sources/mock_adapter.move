module suinergy::mock_adapter {
    ...
    public fun deposit<T>(
        ...
        adapter::new_deposit_result(shares, amount)
    }
    ...
    public fun query_balance<T>(adapter: &MockAdapter<T>, clock: &Clock): u64 {
        // Calculate yield without modifying the adapter
        let current_time = clock::timestamp_ms(clock);
        let elapsed_ms = if (adapter.last_update == 0 || current_time < adapter.last_update) {
            0
        } else {
            current_time - adapter.last_update
        };
        
        let yield_amount = if (elapsed_ms == 0) {
            0
        } else {
            let year_ms = 31536000000; // 365 days in milliseconds
            let balance = balance::value(&adapter.balance);
            ((balance as u128) * (adapter.apy_basis_points as u128) * (elapsed_ms as u128) / 10000 / (year_ms as u128)) as u64
        };
        
        balance::value(&adapter.balance) + adapter.accumulated_yield + yield_amount
    }
    ...
    /// Get position value - implements adapter interface
    /// Returns current value including simulated yield
    public fun get_position_value<T>(
        adapter: &MockAdapter<T>,
        _position_id: ID,
        clock: &Clock
    ): u64 {
        // Return total assets including yield (calculated without modifying adapter)
        query_balance(adapter, clock)
    }
    ...
    public fun get_adapter_info<T>(adapter: &MockAdapter<T>, clock: &Clock): AdapterInfo {
        ...
        adapter::new_adapter_info(
            ...
            @0x0, // Mock adapter identifier
            total_assets,
            adapter.total_shares,
            adapter.apy_basis_points,
            0, // Always excellent for mock
            adapter.last_update,
        )
    }
